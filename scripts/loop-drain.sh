...
# silently-disabled guard is the failure mode to avoid (see the PRD).
BUDGET_ENABLED=1
case "$BUDGET" in
    "" | 0 | 0.0 | -*)
        BUDGET_ENABLED=0
...
while :; do
    ...
    # 2. max-passes.
    if [ "$MAX_PASSES" -gt 0 ] 2>/dev/null && [ "$pass" -ge "$MAX_PASSES" ]; then
        stop_reason="max-passes"
        break
    fi

    # 3. budget — a local proxy for relative burn, not a real quota reading
    # (see ADR 0097 and scripts/lib/usage-window.ts).
    if [ "$BUDGET_ENABLED" -eq 1 ]; then
        usage_json=$(bun run usage:window --hours "$WINDOW_HOURS" --budget "$BUDGET" 2>/dev/null) || usage_json=""
        ...
        if [ -z "$pct" ]; then
            echo "loop-drain: WARNING — could not read a pct from 'bun run usage:window'; skipping the budget check for this pass." >&2
            pct="n/a"
        else
    ...
    # 5. rate limit -> stop and report (never sleep-and-retry — the user
    # chose this explicitly). Detected either from the message shapes above,
    # OR from a non-zero `claude` exit, which is never a normal outcome for
    # this harness and is treated as suspicious on its own.
    rate_limited=0
    ...
    if [ "$rate_limited" -eq 1 ]; then
        ...
        stop_now=1
    elif [ "$queue_after" = "$queue_before" ] && [ "$green_after" = "$green_before" ]; then
        # 6. no-progress — neither the queue nor main's tip moved. Without
        # this, a batch that fails identically every time spins forever.
    ...
    if [ "$stop_now" -eq 1 ]; then
        ...
        if [ "$reason_field" = "rate-limit" ]; then
            echo "loop-drain: rate limit (or non-zero exit $claude_exit) detected on pass $pass — stopping. Log tail:" >&2
