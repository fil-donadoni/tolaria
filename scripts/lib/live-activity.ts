...
interface FileCursor {
    ...
    seen: Set<string>;
    /** The session this file belongs to — what `evictSession` walks. */
    session: string;
}
...
/**
 * The hour bucket a timestamp falls in — its start, epoch ms, floored on
 * FIXED 3,600,000 ms steps (UTC hours). Not the local wall-clock hour: on a
 * DST day local hour starts are not evenly spaced in epoch ms, so a frame
 * generated by stepping `HOUR_MS` back from "now" would miss the keys real
 * messages landed on (review of PR #3136). Fixed steps make the key and the
 * frame the same arithmetic; the LABEL is rendered in local time by the
 * page, which coincides with these boundaries wherever the UTC offset is a
...
export class LiveIndex {
    ...
    private readFile(
        ...
        if (cursor && cursor.offset > size) {
            // Truncated or rotated: everything folded from this session is
            // suspect — the dedupe set would swallow reused response ids and
            // the old totals would stay counted (review of PR #3136). Back
            // the session out entirely; its files are re-read from byte 0
            // on this same refresh.
            this.evictSession(session);
            return this.readFile(path, dir, session, kind, cutoffMs);
        }
    ...
    /**
     * Drop a session and every trace of it: its summary, its per-hour
     * totals (subtracted from the window buckets first), and the cursors of
     * every file it owns. Used on truncation; `prune` uses the cursor half.
     */
    private evictSession(session: string): void {
        const hours = this.sessionHours.get(session);
        if (hours) {
            for (const [hour, sh] of hours) {
                const b = this.buckets.get(hour);
                if (!b) continue;
                b.outTok -= sh.outTok;
                b.inTok -= sh.inTok;
                b.cacheRead -= sh.cacheRead;
                b.cacheWrite -= sh.cacheWrite;
                b.cost -= sh.cost;
                b.messages -= sh.messages;
            }
        }
        this.sessionHours.delete(session);
        this.sessions.delete(session);
        for (const [path, cur] of this.cursors) {
            if (cur.session === session) this.cursors.delete(path);
        }
    }

    /** How many files the index holds a cursor for — bounded by `prune`. */
    ...
    private prune(cutoffMs: number): void {
        ...
        // Cursors of files that aged out of the window go too — a file
        // outside the window has nothing left to contribute, and a
        // long-running dashboard would otherwise hold one `seen` set per
        // transcript it ever opened (review of PR #3136).
        for (const [path, cur] of this.cursors) {
            if (cur.mtimeMs < cutoffMs) this.cursors.delete(path);
        }
...
/** Whether the byte at `pos` is `\n`. */
function newlineAt(path: string, pos: number): boolean {
    const buf = Buffer.alloc(1);
    const fd = openSync(path, "r");
    try {
        return readSync(fd, buf, 0, 1, pos) === 1 && buf[0] === 0x0a;
    } finally {
        closeSync(fd);
    }
}
...
export function readTail(
    ...
    // A mid-file start usually lands inside a line, whose head is then
    // garbage — but not when byte `from - 1` is itself the newline: then
    // the first line is whole and dropping it would lose a real entry
    // (review of PR #3136). One byte read decides.
    const lines =
        from > 0 && raw.length > 0 && !newlineAt(path, from - 1)
            ? raw.slice(1)
            : raw;
