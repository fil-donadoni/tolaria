...
export interface GlossaryEntry {
    /** The human label rendered in place of the raw token. */
    label: string;
    /** What the term COUNTS or MEANS — never a restatement of its name. */
    tip: string;
}
...
export const GLOSSARY = {
    ...
    pri: {
        label: "priority",
        tip: "The issue's Priority field on the project board — P0, P1, P2, P3. The queue sorts on the BAND first — the parent PRD's Priority when the PRD carries one, else this — then standalone-before-slice, then on this (issue #4371). An em dash means the board has no priority set.",
    },
    queue: {
        label: "queue depth",
    ...
    "queue.P0": {
        label: "P0",
        tip: "Unclaimed ready-for-agent issues in the P0 BAND — their parent PRD's Priority is P0, or they have no prioritised parent and their own is. Picked before anything else.",
    },
    "queue.P1": {
        label: "P1",
        tip: "Unclaimed ready-for-agent issues in the P1 band — their parent PRD's Priority is P1, or they have no prioritised parent and their own is.",
    },
    "queue.P2": {
        label: "P2",
        tip: "Unclaimed ready-for-agent issues in the P2 band — their parent PRD's Priority is P2, or they have no prioritised parent and their own is.",
    },
    "queue.P3": {
        label: "P3",
        tip: "Unclaimed ready-for-agent issues in the P3 band — their parent PRD's Priority is P3, or they have no prioritised parent and their own is. Deliberately last, which is NOT the same as no priority set.",
    },
    "queue.unprioritized": {
        label: "no priority",
 ...
 * `llm.messages`).
 *
 * `hasOwnProperty`, never a bare `GLOSSARY[term]`: `constructor`, `__proto__`
 * and `toString` are inherited truthy values on a plain object, and a table
 * asked about names the SERVER supplies must not answer for them.
 *
 * Takes a `string`, not a `TermId`: the qualified form is composed at runtime
 * from a dataset name, and answering for keys that are NOT in the table is the
 * whole point of the fallback. `Term` is the typed door; this is the runtime
 * one.
 */
export function lookupTerm(term: string): GlossaryEntry | undefined {
    if (typeof term !== "string" || term === "") return undefined;
    const table: Record<string, GlossaryEntry> = GLOSSARY;
    if (Object.prototype.hasOwnProperty.call(table, term)) return table[term];
    const dot = term.indexOf(".");
    if (dot === -1) return undefined;
    const bare = term.slice(dot + 1);
    return Object.prototype.hasOwnProperty.call(table, bare)
        ? table[bare]
        : undefined;
}
 ...
 */
export function labelFor(term: string, scope?: string): string {
    const key = scope ? `${scope}.${term}` : term;
    return lookupTerm(key)?.label ?? String(term).replace(/_/g, " ");
}

/** Every key the table actually declares — the type-level half of the
 *  completeness guarantee (PRD #3148 S1). */
