...
**Split Card**:
One card printed with two castable **Halves** (CR 709.2 — drawing or discarding it draws or discards ONE card). In every zone except the **Stack** its characteristics are those of both halves **combined** (CR 709.4): two names, the summed **Mana Cost**, and every card type from either half — so Life // Death is a green and black card with mana value 3 in a **Graveyard**. The combination is DERIVED from the halves, never authored beside them (ADR 0121). A split card whose halves are **Permanent** cards sharing one type line is a **Room** (CR 709.5), and its combination is a function of which **Doors** are unlocked rather than a constant (ADR 0123).
_Avoid_: Double-faced card, **Inset Spell** (that is one card with a normal form, CR 715), "the two cards", modal card

**Split Half**:
One of the two castable objects a **Split Card** offers. A player chooses which half they are casting BEFORE the spell is put onto the **Stack** (CR 709.3), only that half is evaluated for legality (CR 709.3a), and while on the stack only its characteristics exist (CR 709.3b) — so the other half's cost, type and text are not merely unused, they do not exist. For a **Room** the halves still EXIST on the stack even so (CR 709.5b, an explicit exception to 709.3b), which is what makes a copy of a Room spell still a two-doored Room; what the copy does not get is the locked half's characteristics. There is no cast that puts the combined card onto the stack. A copy of a split spell is still a split card with both halves (CR 709.3c).
_Avoid_: Side, face, mode (a **Modal** spell is one object with chosen modes, CR 700.2), **Adventure**

**Room**:
A **Split Card** whose two **Halves** are **Permanent** cards sharing one type line (CR 709.5) — every printed one is `Enchantment — Room`. Unlike an ordinary split card, whose combination is fixed, a Room's characteristics are a function of which **Doors** are unlocked: the shared type line carries two static abilities saying the permanent does not have the name, mana cost or rules text of a locked half, and those abilities are part of its **Copiable Values**. The type line itself is shared and always present, so a Room with both doors locked is a nameless, costless, textless `Enchantment — Room` (CR 709.5a).
_Avoid_: Enchantment-split, locked split card, "the Room card" for one half (that is a **Door**)

**Door**:
One **Half** of a **Room** (CR 709.5j — "a 'door' of a Room permanent … is a half of that permanent"). The word the cards use; the rule's word is half, and the two are the same thing. A door is _unlocked_ when the permanent has the matching **Unlocked Designation** and _locked_ otherwise (CR 709.5c).
_Avoid_: Side, face, mode, room (a Room is the whole permanent, its doors are the halves)

**Unlocked Designation**:
A designation a **Room** permanent can have, one per **Door** — "left half unlocked" and "right half unlocked" (CR 709.5c). Unlike the **Monarch** and the city's blessing, which a PLAYER holds, these are held by a permanent, and they are part of its **Copiable Values**, so a copy of a half-open Room is half-open the same way. A permanent enters with the designation of the door that was cast, and with neither if neither was (CR 709.5d). A player may pay a locked door's mana cost as a **Special Action** to gain the matching designation, only in their own main phase with an empty **Stack** while they hold **Priority** (CR 709.5e).
_Avoid_: Unlock counter, door flag, room state, "opened"
...
**Cast Permission**:
A **Static Ability** on a battlefield permanent that lets a player **Cast** a whole CLASS of cards from hand on terms the sentence states — without paying their mana costs, and/or as though they had flash (CR 601.3 "a rule or effect allows that player to cast it", CR 118.9 for the cost half). Aluren is the archetype; Vedalken Orrery and Dracogenesis are the two halves alone. The grant-polarity sibling of a cast RESTRICTION, and read the same way: a declarative card filter plus two independent booleans, evaluated at cast time, never materialised onto the permitted card. Its `id` is DERIVED from the clause rather than named after the card (ADR 0119) — the engine deduplicates permissions on that id across both battlefields, so the id identifies the offered cast OPTION, and two cards printing one sentence must offer one option.
_Avoid_: Cast grant, free-cast static (the free half is only one of the two terms, and a permission may grant neither the cost nor the timing half exclusively)
...
**Quarantine Class**:
Which of two kinds a reason is when the generated smoke scenario cannot run a **Compiled Definition**'s script. **Op-covered**: the Op's own mechanism stops the scenario (it waits for a decision, sets up a shield for later, draws a random bit), the same for every card that uses it, so the Op's own permanent test is the evidence and the card is not withheld. **Card-dependent**: what the card's clause gives the Op stops it (an amount read at runtime, an object in a zone the scenario does not set up), so the card stays in **quarantine** until a golden fixture of the **Grammar Rule** shows that same form compiling correctly. The card ITSELF as subject is card-dependent only where the scenario cannot seed the source — a spell site, or an ability whose source is not on the battlefield when it resolves (a trigger on its own death, an activation from a graveyard or a hand) — or where the Op's primitive refuses the kind the card actually is; at an ordinary ability site the scenario seeds the source, as the card, and runs the clause against it. Every reason has exactly one class, and a doubtful one is card-dependent.
...
**Mass Subject**:
A subject that names every permanent a descriptor matches without announcing a target — "all enchantments", "all other creatures you control", "each artifact, creature, and enchantment with mana value X or less" (CR 110.1) — read by the verbs that fan out (destroy, tap, untap) and lowered to a `forEach` over the battlefield with the verb reading `$each`. Only the descriptor fields the selector expresses one-to-one are accepted (card types, subtypes, "you control", "other"); any other field refuses the line rather than being dropped, and a noun list mixing card types with subtypes, or stacked subtypes, is refused because a selector ORs within a field and ANDs across fields. An `X` bound is an `if` on the swept permanent's mana value inside the sweep, since the selector has no mana-value field. Read by the `mass subject` sub-grammar.
_Avoid_: Mass removal, board wipe, sweeper (those name the card's role, not the phrase the grammar reads), target set
...
**Hand Tail**:
The cards of a **Target List** whose residual **Grammar Gaps** all sit below the hand-tail floor — a gap carried by fewer corpus cards than the floor (initially three) is a per-card script in grammar's clothing (breadth, not the cards it alone would turn `ready`), so the card is written by hand instead, one issue per card, and carries a terminal `hand-tail:` marker naming its fragment. A state, never a sentence: the **Oracle Lockfile** compiles every corpus card, hand-written ones included, so a Hand Tail card whose row turns `ready` is migrated (**Card Retirement**), and one whose gaps climb back above the floor re-enters the grammar queue — both red on `health` until acted on. A protocol card (a closure body) is Hand Tail by construction: its `ready` row proves the text compiles, not that the behaviour matches, so it is never migrated automatically — it is surfaced for a manual behaviour comparison, and leaves the tail only when a person replaces the closure. A Target List is complete at 100% playable with its Hand Tail declared by name, never at 100% grammar.
...
_Avoid_: Status, maturity, stable/unstable, alpha
...
**Draft Signal**:
What the cards still present in a passed-around pack say about which colours — and, where cards carry **Card Profiles**, which **Archetypes** — the upstream **Seats** are not taking. Read as a running gap between what a pack still holds and what a pack of that size drawn from the same scope would be expected to hold, measured in raw card power (never the fit score the Signal then feeds into, which would make the read circular). A pack nobody has picked from yet says nothing. Kept as one read per passing direction, because the Seats feeding a drafter change every round: what a drafter learns about one side in the first round pays off in the third, not in the second. A Signal yields to **Colour Commitment** rather than fighting it: it counts for as much as the Seat is still undecided, so a drafter with no direction follows the open colour and a settled one barely notices it. A multicolour card counts partly toward each of its colours; a card with no Card Profile says nothing about Archetypes, so a set without profiles is read by colour alone. In the **Vintage Cube** the Archetype read usually matters more than the colour read — a control deck and an aggro deck of the same colours want different cards.
...
**Cut Experiment**:
A paired comparison run on identical seeds: one table where an upstream **Seat** is forced to take only one colour or **Archetype**, one where it drafts normally, asserting that the Seat downstream ends with less of what was cut. The evidence that a **Bot Drafter** actually reads **Draft Signals**, where a **Pick Invariant** can only prove the read is built right. Openly an opinion in the same sense as an **Anchor Pick** — it holds for sensible weightings, not for every one — so a red one calls for a decision, not a weight fix.
_Avoid_: Signal test, simulation
...
**Cast Commit**:
The one server step that turns an answered **Cast** announcement into a **Spell** on the **Stack**: pay the mana leg and capture what it produced (the riders, the mana actually spent, the one-shot substitution grant it consumed), pay every other cost leg the announcement answered (life, the hand leg, the graveyard exile picks, convoke, the filtered sacrifice or exile), spend the **Graveyard Play Permission** that licensed it, move the card out of the **Zone** it is cast from, build the stack item with its cost record and its **Cast Provenance**, apply the announced cast mode's characteristics, place the item and announce the cast (CR 601.2h–i). It takes a plan whose legs are already answered and applies it — it never decides WHICH payments a cast owes; that stays the **Payment Park** commit gate, the one owed-payment seam. Every server path that commits a cast — the deferred park commit, the immediate announcement branches, the targeted commit and a cast made during **Resolution** — is a caller, so a new cost leg or stack-item stamp is written once. The search has its own kernel of the same shape, and the two are pinned to produce the same cost record.
_Avoid_: Commit path, commit site (there is one), cast sequence, "put on the stack" (that is one of its steps), **Announce** (that is the first step of casting — the commit is the last)
