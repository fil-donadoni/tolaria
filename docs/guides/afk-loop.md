...
still escalates above the session tier on its own (`/next-issue` §4).

**If `queue:plan` cannot answer, the run STOPS** (`preflight-error`). This is
the one place the driver's usual "a janitor must not end the night" trade goes
the other way, and the HITL rule above is why: a [pass](#g-pass) handed a bare
prompt picks its own issue, and `/next-issue` has no notion of HITL, so it
would implement and then merge work reserved for a human. Losing the rest of a
night is the cheaper failure — and `gh-error` already stops the run for the
strictly milder problem of not being able to _count_ the queue.
...
| `preflight-error` | the planner could not name an eligible head issue                                            | check `bun run queue:plan --cap 1 --exclude-hitl`. Deliberately fatal: a pass with no issue handed to it picks its own, and would not know to leave HITL work alone                                                                                                                                                                                             |
