...
Non è più un lavoro a mano. Con il grammar-first (ADR 0137) il compilatore
Oracle è il percorso di authoring, e una carta scritta a mano si ritira quando
il suo gemello compilato fa round-trip (verdetto Guard C `equal`, ADR 0114):
`gaps:sync` apre una issue di kind `migration` per ogni Grammar Rule che fa
graduare carte (es. issue #4027, 108 carte). Il classificatore
`migration-classifier.mjs`, il playbook di migrazione a mano e i due gate
storici — #1435 ("FREE → 0") e #1438 ("Op-blocked → 0") — sono ritirati
(issue #4107).
