...
1. **`comboAnnotations.ts` is deleted.** No per-card or per-combo boost layer
   ...
   lands, then become `must`. (Corrected when this was carried out, issue
   #3138: this said "the **three** Twin/Exarch entries", but only two are
   `stretch` — issue #2469 had already promoted the granted-Grizzly-Bears
   entry to `must`, and it stays green through the deletion: blade `must`
   tier 106/106 before and after.)
