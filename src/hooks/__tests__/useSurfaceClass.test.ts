// `useSurfaceClass` (#2585) — the OVERLAY-shape seam: phone / tablet / desktop.
//
...
import { useSurfaceClass } from "../useSurfaceClass";
...
describe("useSurfaceClass vs useViewportMode (#2585)", () => {
    ...
    it("disagrees with useViewportMode on a portrait tablet", () => {
        ...
        expect(surface.result.current).toBe("tablet");
    });
    ...
    it("agrees that a real desktop is a desktop", () => {
        device = { width: 1440, height: 900, pointer: "fine" };
        const surface = renderHook(() => useSurfaceClass());
        const layout = renderHook(() => useViewportMode());
        expect(layout.result.current).toBe("desktop");
        expect(surface.result.current).toBe("desktop");
    });
});
...
describe("useSurfaceClass — boundaries and liveness (#2585)", () => {
    ...
    // an order that tested `pointer: coarse` first would call every phone a
    // tablet and hand it an anchored popover on a 390px-wide screen.
    it("a coarse phone is a phone, not a tablet", () => {
        device = { width: 390, height: 844, pointer: "coarse" };
        const { result } = renderHook(() => useSurfaceClass());
        expect(result.current).toBe("phone");
    });

    // A fine-pointer laptop is never a tablet however narrow the window gets —
    ...
    it("a narrow fine-pointer window is desktop until it is phone-shaped", () => {
