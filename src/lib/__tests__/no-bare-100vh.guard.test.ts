...
//
// Scope: every real `100vh` height-cap site in `src/` was converted to
// `100dvh` (issue #2594) — `game-dialog.tsx`, `controller-phase-list.tsx`,
// `mana-choice-picker.tsx`, `phyrexian-picker.tsx`, `mode-picker.tsx`,
// `card-preview-anchored.tsx`, `additional-cost-picker.tsx`,
// `alt-cost-picker.tsx`, `dev-panel-rail.tsx`. This guard is what keeps the
// next one from landing unnoticed.
//
...
//   - prose (`//` / `*` comment lines) that MENTIONS the literal string
//     "100vh" to explain the bug or as Tailwind-syntax documentation
//     (`shellLayout.ts`'s own doc comment, `inspect-overlay.tsx`'s doc
//     comment) — these are not live height caps;
...
describe("no bare 100vh height caps outside comments (issue #2594)", () => {
    it('every literal "100vh" in src/ is either a comment or on the documented allowlist', () => {
