...
export default function DraftLabReplayDivergenceBanner({
    ...
    return (
        <p className="rounded-sm bg-signal-opponent/10 px-2 py-1.5 text-[11px] text-signal-opponent">
            Faithful through pick {result.firstDivergedPickIndex - 1} of {total}
            . From pick {result.firstDivergedPickIndex} on, the current scorer
            would have picked differently ({divergedCount} of {total} pick(s)
            moved) — every later pick shown below is still computed against the
            REAL historical packs, which a fully retuned redraft would not
            actually have seen past this point, so treat everything from here on
            as illustrative, not a faithful reconstruction.
        </p>
    );
}
