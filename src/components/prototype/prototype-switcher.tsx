...
export default function PrototypeSwitcher({
    ...
    return (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-[9999] flex items-center gap-1 rounded-full bg-white px-1.5 py-1 text-neutral-900 shadow-[0_6px_24px_rgba(0,0,0,0.5)] ring-1 ring-black/20">
            <button
                type="button"
                onClick={() => go(-1)}
                className="h-8 w-8 rounded-full text-lg leading-none hover:bg-neutral-200"
                aria-label="Previous variant"
            >
                ‹
            </button>
            <span className="min-w-[13rem] text-center text-sm font-semibold tabular-nums">
