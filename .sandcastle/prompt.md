...
4. **Verify** — run these commands and fix any failures before proceeding:
    - `bun run check:all` — format + lint + type-check (zero errors required)
    - `bun run test` — vitest suite (zero failures required)
