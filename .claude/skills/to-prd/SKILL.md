...
3. Write the PRD using the template below, then publish it to the project issue tracker as the umbrella issue. Apply the `prd` label so the umbrella is identifiable as a PRD. If the `prd` label does not yet exist in the tracker, create it first (e.g. `gh label create prd --description "Umbrella PRD issue" --color 0e8a16`).
