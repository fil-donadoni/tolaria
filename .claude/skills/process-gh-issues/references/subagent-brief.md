...
6. **Pre-PR gate (light, mandatory).** When the implementation is complete, run: (a) the **targeted tests** for everything the diff touches (the issue's own tests + the suites of the modules it modifies), and (b) the project's **complete fast static checks — never a hand-picked subset**. In Tolaria that is exactly **`bun run check:pr`**: `check:all:inner` as the full gate runs it (format + lint + type-check + `check:index` + `check:stubs`) PLUS `check:guards` (the bot fast lane, the whole `node` **and `dom`** vitest projects, `cr:lint`) — all on the unlocked light tier. Picking `check:ts && lint` and dropping the rest saved nothing (those checks cost <0.2s each) and made every card-shipping PR fail at the merge-train on the card-index lockfile guard. Since #2655, `check:pr` also covers a `src/` diff BEFORE the PR opens: `check:guards` runs the `dom` project whole (not a diff-derived subset — #2584 died on a census guard the failing diff never touched), on the same light tier, no mutex. Do **not** run the full suite or full `check:all` on the branch — the merge-train (SKILL.md §4 step 3) runs the full gate once on the rebased tree that actually lands, and a per-branch full gate would be re-paid there. Do not open the PR until this light gate is green.
