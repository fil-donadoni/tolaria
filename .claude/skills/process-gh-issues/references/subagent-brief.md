...
6. **Pre-PR gate (light, mandatory).** When the implementation is complete, run: (a) the **targeted tests** for everything the diff touches (the issue's own tests + the suites of the modules it modifies), and (b) the project's **complete fast static checks — never a hand-picked subset**. In Tolaria that is **`bun run check:lane`** (#2738/#2741/#2743) — it classifies the diff into `skin`/`engine`/`full` and runs exactly that lane's checks, printing a per-check receipt. On a diff it cannot affirmatively place in `skin` or `engine` (mixed, `package.json`, a lockfile, `.claude/**`, an unrecognised path) it falls back to **`bun run check:pr`** verbatim, which is what `check:lane` always runs for the whole repo's worth of change anyway: `check:all:inner` as the full gate runs it (format + lint + type-check + `check:index` + `check:stubs`) PLUS `check:guards` (the bot fast lane, the whole `node` **and `dom`** vitest projects, `cr:lint`) — all on the unlocked light tier. Picking `check:ts && lint` and dropping the rest saved nothing (those checks cost <0.2s each) and made every card-shipping PR fail at the merge-train on the card-index lockfile guard. **No lane ever scopes a project's tests to a diff-derived subset** — `skin` runs the WHOLE `dom` project (not a diff-derived subset — #2584 died on a census guard the failing diff never touched), `engine` runs the WHOLE `node` project; `skin`'s `node[src,scripts]` is the one exception, and it is scoped to a fixed, declared `src/ scripts/` written into the lane, never to the changed files. The diff decides whether a project runs at all, never a diff-derived slice of it (ADR 0104). Do **not** run the full suite or full `check:all` on the branch — the merge-train (SKILL.md §4 step 3) runs the full gate once on the rebased tree that actually lands, and a per-branch full gate would be re-paid there. Do not open the PR until this light gate is green.
...
9. **Write the receipt to the batch artifact directory, then return a one-line summary.** The receipt is a FILE, not a paragraph — `.claude/receipts/<BATCH_ID>/<issue>-implement.json`, written through `writeReceipt` (`scripts/lib/receipt.ts`), which validates before it writes and rejects a malformed receipt naming the offending field. `BATCH_ID` arrives in your prompt.
    ...
    **A sibling refusal, same file, different trigger.** `writeReceipt` also throws when the batch directory you were handed does not exist yet AND another batch already holds receipts for this same issue — the shape of being handed the wrong `BATCH_ID` (a typo'd session id would otherwise mint a new, empty-looking directory that `queue:train` never reads). If you hit this and it is genuinely a fresh re-attempt of the issue in a new batch, not a wrong id, set `TOLARIA_ALLOW_RECEIPT_REBATCH=1` and re-run — `writeReceipt`'s own escape hatch (`scripts/lib/receipt.ts`).
