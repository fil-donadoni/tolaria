...
**The verdict is a receipt, written to the same batch directory** as `<issue>-review.json` (`role: "review"`, `outcome: "approve" | "blocking"`, `pr`, `findings[]`) before the reviewer returns its one-line summary. **`findings` is `string[]` — plain strings, one line per finding, NOT objects.** `{ id, severity, where, problem, ... }` shaped entries are the `ReportFindings` tool's contract, a different schema — writing that shape here fails `requireStringArray` at the write (`receipt.findings[0]: expected a non-empty string`) and the issue silently drops out of the train's plan. Fold each finding's file/line/severity/fix into ONE prose line instead, e.g. `"convex/gre/search.ts:709 (medium) — eternalize is enumerated but never chosen by search; applyMoveInSearch never pushes the ability onto the stack, so it scores as a pure loss. Not fixable in this PR's scope (#1920); disclose in the PR body."`. A `blocking` verdict with an empty `findings` list is rejected by the contract — it is the shape that stalls a train with nothing to hand the fixup subagent. Persisting the verdict is what makes "was this PR reviewed?" answerable after an interrupt, instead of a question only the dead context could answer.
