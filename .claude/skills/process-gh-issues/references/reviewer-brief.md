...
**The verdict is a receipt, and you write it by RUNNING A COMMAND — never by hand-authoring the JSON file.** `scripts/write-review-receipt.ts` (`bun run review:receipt`) is the ONLY sanctioned way to produce `<issue>-review.json`: it builds the object and calls `writeReceipt` (`scripts/lib/receipt.ts`) itself, so there is no path through it that can produce a missing `version` or a `findings` entry that isn't a plain string — the two exact shapes that made 4/4 review receipts malformed in the batch that first reported this (#2285), while every implement/fixup receipt in the same batch was fine because that subagent was already writing TypeScript. Naming `writeReceipt` in prose a second time would not have fixed that — a sentence describing a schema is not a validator, and a reviewer satisfying the sentence can still produce something `parseReceipt` rejects. **If you find yourself typing `{ role: "review", ... }` into a file, stop hand-authoring it** — run the command instead:

```
bun run review:receipt --batch "$BATCH_ID" --issue <N> --pr <PR> --outcome approve

bun run review:receipt --batch "$BATCH_ID" --issue <N> --pr <PR> --outcome blocking \
    --finding "convex/gre/search.ts:709 (medium) — eternalize is enumerated but never chosen by search; applyMoveInSearch never pushes the ability onto the stack, so it scores as a pure loss. Not fixable in this PR's scope (#1920); disclose in the PR body." \
    --finding "<second finding, one prose line each>"
```

Run it before returning your one-line summary. `--finding` takes exactly one prose line per finding — fold file/line/severity/fix into that ONE line; repeat the flag for each additional finding. `{ id, severity, where, problem, ... }`-shaped entries are the `ReportFindings` tool's contract, a different schema — the CLI has no argument shape that could produce that, so there is nothing to get wrong here. A `blocking` verdict with zero `--finding` flags is rejected (exit non-zero, message from the contract) — it is the shape that stalls a train with nothing to hand the fixup subagent, and the CLI enforces this by calling the same `writeReceipt` the schema owns, not by re-implementing the rule. Persisting the verdict is what makes "was this PR reviewed?" answerable after an interrupt, instead of a question only the dead context could answer.

**Re-reviewing a fix? The CLI's `--round` flag is how a second verdict avoids colliding with the first** — `writeReceipt` refuses to overwrite the round-1 file. If you are reviewing a fixup that answers YOUR OWN earlier blocking verdict (or anyone else's) for this same issue, pass `--round` one higher than what is already on disk (`ls .claude/receipts/<BATCH_ID>/ | grep '^<issue>-review'` tells you the highest round present; round 1 has no suffix) — e.g. `bun run review:receipt --batch "$BATCH_ID" --issue <N> --pr <PR> --outcome approve --round 2`, landing at `<issue>-review-2.json`. The train selects the effective verdict by round, not by which file it happens to read first, so this is what lets your approve supersede the earlier block rather than being hand-edited over it or silently losing to it on disk. The common case — one review, ever — needs no `--round` at all.
