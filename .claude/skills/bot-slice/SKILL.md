...
| **A new `evaluate` term (`EvalTerms` key)** | its row in `src/lib/ai/eval-term-labels.ts` — the ONE table the DecisionTrace line AND its legend render from (`Record<keyof EvalTerms, …>`, so `tsc` reds on a missing row; runtime twin in `eval-term-labels.bot.test.ts`). Miss it and the term is invisible in the trace and the self/opp line stops summing to the Δ printed beside it — which is how #2686's `manaDevelopment` shipped unseen |
