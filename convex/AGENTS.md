...
- **Guard C — round-trip-or-declare-the-gap (#2701, PRD #2693).** The gap the
  ...
  **`// compiler-gap: <fragment> (#issue)`** in the comment paragraph directly
  ...
  reds. Sibling `// hand-tail: <fragment> (#issue)` (issue #3867): same checks, a card
  hand-written by decision. (3) Sits in the one-time **baseline**
...
`identity-only-card-tests.test.ts` reds one in `cards/sets/**` at PR time;
repo-wide, health's `check:test-hygiene` reds it unless named in
`scripts/lib/identity-test-allowlist.json`.
...
| `OP_VALUERS` / `OP_BENEFICENCE`                        | does it know it WANTS to? | both censused in the bot suite; a `"neutral"` sign needs a written reason (issue #3006) |
