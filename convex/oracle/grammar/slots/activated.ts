...
export const activatedSlot: Rule<SlotIR> = rule(ACTIVATED_SLOT, (span, ctx) => {
    ...
    // permanent the Aura is attached to. A granted ability's context carries
    // the HOST's type line (`hostTypeOnly`), whose "Aura" is never the
    // source's, so it never raises the flag.
    const auraHost =
        context.typeLine.hostTypeOnly !== true &&
        context.typeLine.subtypes.includes("Aura");
