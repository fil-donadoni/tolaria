...
import { getFlashbackAdditionalCost } from "./gre/flashback";
import { countDistinctCardTypes, hasEscape } from "./gre/escape";
import { RETRACE_COST_LEGS } from "./gre/retrace";
import {
    applyGenericOffset,
...
import { recomposeLayer6ForInstance } from "./gre/layer6";
import {
...
// same way.
import { isTapOtherPaid, tapOtherContribution } from "./gre/owedPayment";
...
import { payLoyaltyCost } from "./gre/loyalty";
...
// moved to `gre/activation.ts` (issue #3479): this module imports `./auth`, so
// anything sharing a file with it is server-only and can never be replayed in
// the browser (ADR 0074). Re-exported here so every existing importer — the
// mutations below, and the tests that pull these from `"../../game"` — keeps
// working against the SAME function objects, which is why there is no second
// activation path to drift from.
export {
 ...
 *  depletion land.
 *
 *  The single authority for the restore, over all FIVE reversal sites — three
 *  on the activation side (`untapSourceFromPayment`, `tapUntap`'s choice
 *  branch, `tapUntap`'s fixed branch) and two on the cast side
 *  (`untapForPayment`, `rollbackPendingCast`). Before this helper existed
 *  there were two hand-rolled copies and three holes, and each hole burns the
 *  counter permanently: the land yields its mana once instead of twice, for a
 *  cast that never happened. The cast-side pair is the same
 *  reversal-symmetry class #3214 and #3354 already fixed for the exert and
 *  life/mana legs.
 *
 *  CR 122.1b (issue #1194) — a KEYWORD counter coming back re-grants its
 *  keyword, and the layer-6 recompose makes it visible before the next SBA
 *  pass, mirroring exactly what `payRemoveCounterCost` splices out when the
 *  cost is paid. That is why this takes `state`: a restore that put the
 *  counter back without the grant would leave a permanent holding an
 *  indestructible counter and not indestructible. No-op when the tap removed
 *  no counters. */
export function restoreManaCounterRemovalOnUntap(
    ...
    applyKeywordCounterGrant(state, card, type);
 ...
 *  are mutually exclusive on one ability — asserted catalogue-wide by
 *  `manaAbility.catalogue.test.ts`, because nothing else stops a future card
 *  declaring both and having the second stamp clobber the first — and sharing
 *  the field means all five reversal sites reverse it through the one
 *  `restoreManaCounterRemovalOnUntap` authority.
 *
...
function refundChosenManaOutput(
    ...
    const riders = manaRidersForAbility(producingAbility);
    // CR 106.6 (issue #3354) — symmetric with `depositTappedMana`: a rider
    // banked the mana in the parallel pool even without a restriction, so the
    // reversal must look for it there or the refund silently untaps for free.
...
function refundFixedManaOutput(
    ...
    // CR 106.6 (issue #3354) — the fixed-output twin of the split in
    // `refundChosenManaOutput`: an ability whose mana carries a rider banked
    // into the parallel pool whether or not it also carried a restriction.
    const riders = manaRidersForAbility(getActivatedManaAbility(card));
    if (restriction || hasAnyManaRider(riders)) {
        reverseRestrictedManaFromPool(
            player,
            manaColor,
            amount,
...
export function tapSourceIntoPayment(
    ...
    if (
        ...
        depositTappedMana(
            ...
            manaRidersForAbility(effAbility)
        );
    ...
    depositTappedMana(
        ...
        manaRidersForAbility(multiColorAbility ?? getActivatedManaAbility(card))
    );
...
function untapSourceFromPayment(
    ...
    restoreManaCounterRemovalOnUntap(state, card);
...
// Cast zone, cast cost and cast stack flags now live in `gre/castCost.ts` — the
// Bot's move enumerator needs the same answers and cannot import this module
...
function rollbackPendingCast(state: GameState): void {
    ...
    for (const cardId of state.pendingCast.tappedLandIds) {
        ...
        restoreExertOnUntap(state, card);
        // CR 106.4 / 118.3 / 122.1 — and the counters the tap's own
        // `removeCounter` leg took (a depletion land, a Mana Battery's scaling
        // cost). Same reversal-symmetry class as the exert leg above: this
        // cast-side loop carries its own inline copy of the untap, so every
        // reversal `untapSourceFromPayment` runs has to be restated here or a
        // cancelled cast burns the counter permanently — the land then yields
        // its mana once instead of twice, for a spell that was never cast.
        restoreManaCounterRemovalOnUntap(state, card);
            ...
            const candidates = player.battlefield.filter((c) =>
                ...
                matchesPermanentFilter(
                    ...
                    ability.cost.sacrificeFilter!,
                    {
                        ...
                        // (`hostOfSource` fails closed without the host).
...
export const untapForPayment = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        restoreLifePaidOnUntap(player, card);
        restoreManaPaidOnUntap(player, card);
        // CR 106.4 / 118.3 / 122.1 — and the counter leg, the third member of
        // that same class: undoing ONE payment tap must not spend a depletion
        // land's counter (or a Mana Battery's charge counters) on a cast the
        // player then abandons. One misclick — tap the land, click it again —
        // was enough to burn it.
        restoreManaCounterRemovalOnUntap(state, card);
        discardPermanentTappedEvent(state, card.id);
    ...
    handler: async (ctx, args) => {
        ...
        if (
            ...
            if (!wasTapped) {
                ...
                // CR 106.6 riders (issue #1559 Delighted Halfling / #3354
                // Arena of Glory) — carried onto the deposited unit alongside
                // the restriction, and on their own when there is none.
                // Routed through the shared `depositTappedMana` rather than a
                // fourth hand-rolled copy of the same branch: the "a rider
                // alone also banks in the parallel pool" rule must hold at
                // every deposit site or the two halves disagree.
                depositTappedMana(
                    player,
                    chosen,
                    effAbility?.manaRestriction,
            ...
            if (manaColor) {
                ...
                if (!wasTapped) {
                    ...
                    depositTappedMana(
                        ...
                        added,
                        multiColorAbility.manaRestriction,
                        manaRidersForAbility(multiColorAbility)
                    );
                    producedThisActivation = added;
                    emitPermanentTapped(
                        state,
    ...
    args: {
        ...
        cards: v.array(
            v.object({
                 ...
                 *  one-shot turned into a creature. The shape is the engine's
                 *  own `AnimateSpec` and the builder re-EXECUTES it through the
