...
export const createManualSoloGame = mutation({
    ...
    handler: async (ctx, args) => {
        if (args.deck.format !== "manual")
            throw new Error(
                "Only manual-format decks can start a Manual Game."
            );
        const user = await getCurrentUser(ctx);
        if (await findActiveMatchForUser(ctx, user._id))
            throw new Error(ACTIVE_GAME_MESSAGE);
        ...
        const deckCards = args.deck.cards.map((c) => ({
            cardId: c.cardId,
            cardName: c.cardName,
        }));
        ...
        const gameId = await ctx.db.insert("games", {
            ...
            status: "playing",
            players: [
                { ...player1, deck: args.deck },
                { ...player2, deck: args.deck },
            ],
            solo: true,
            mode: "manual",
            createdAt: now,
        ...
        await saveManualState(ctx, gameId, 0, state, null);
        appendManualLog(ctx, gameId, {
            text: "Manual game started",
            timestamp: now,
        });
