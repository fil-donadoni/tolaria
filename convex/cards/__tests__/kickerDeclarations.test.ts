...
// and a DSL script reads one Kicker with `{ additionalCostPaid: "<id>" }`. Three failure
...
//  2. **A `{ additionalCostPaid: "<id>" }` naming no declared Kicker.** The value reads
...
/** Every `{ additionalCostPaid: "<id>" }` id reachable anywhere in a card's declaration —
 ...
 *  still found. */
function additionalCostPaidIdsIn(value: unknown, out: Set<string>): void {
    if (Array.isArray(value)) {
        for (const v of value) additionalCostPaidIdsIn(v, out);
        return;
    }
    if (typeof value !== "object" || value === null) return;
    const rec = value as Record<string, unknown>;
    if (typeof rec.additionalCostPaid === "string")
        out.add(rec.additionalCostPaid);
    for (const v of Object.values(rec)) additionalCostPaidIdsIn(v, out);
}
...
describe("Kicker declarations (CR 702.33 / 702.33e, ADR 0079)", () => {
    ...
    it.each(cardsWithKickers.map((c) => [c.name ?? c.id, c] as const))(
        "%s — every { additionalCostPaid } names a declared kicker (fail-closed trap)",
        (_name, card) => {
    ...
    it("no card outside `kickers` references a kicker by id", () => {
        // A `{ additionalCostPaid }` on a card with NO kickers always reads 0 — the
    ...
    it("the trap reaches mode sites and aiEffects shadows (site enumeration is complete)", () => {
        // The sweep is only as good as its site list: a `{ additionalCostPaid }` hiding
                ...
                aiEffects: [
                    {
                        op: "if",
                        predicate: {
                            left: { additionalCostPaid: "from-ai-shadow" },
                            op: "ge",
                            right: 1,
                        },
                        then: [],
                        ...
                        effects: [
                            {
                                op: "if",
                                predicate: {
                                    left: { additionalCostPaid: "from-mode" },
                                    op: "ge",
                                    right: 1,
                                },
                                then: [],
