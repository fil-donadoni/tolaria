...
//  `getStackAbilities`) fired on an empty hand — the ability still showed as
//  activatable and, once activated, silently granted no counter. Modelling
//  the discard as `cost.discardFilter` fixes both: activation is illegal on
//  an empty hand, so the counter can never land without paying the cost.)
