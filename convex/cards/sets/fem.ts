...
// ─────────────────────────────────────────────────────────────────────────────
// C6 — Artifacts & Lands (PRD #566, issue #573).
//
// Colourless artifacts plus the FEM cross-colour mana base. New engine
// capabilities concentrated here:
//   • CAPABILITY H — variable counter-removal producing variable mana. Reuses
//     the shipped `manaChoiceRemovesCounters` seam (Mana Batteries, LEG):
//     a `useStack:false` tap mana ability whose chosen index N removes N
//     `storage` counters and adds N mana of the land's colour (CR 122.6 /
//     605.1a). The storage-land cycle.
//   • CAPABILITY B — control-change-on-tap (Rainbow Vale): a `useStack:false`
//     tap mana ability that adds one mana of any colour and arms a
//     `next-end-step` delayed trigger (the new `armsDelayedTriggerOnTap` seam,
//     ADR 0040) calling INDEFINITE `gainControl` to the opponent. Ping-pongs on
//     repeated activations. Precedent: Ghazbán Ogre's indefinite gainControl.
//   • REUSE C — sacrifice-self mana ability (ADR 0039, FEM C5 Basal Thrull /
//     LEA Black Lotus): the sacrifice-land cycle + Implements of Sacrifice.
//   • REUSE I — optional "may choose not to untap" (FEM C2 Seasinger, ATQ): the
//     storage lands, Spirit Shield, and Zelyon Sword.
//
// All data validated against Scryfall `set:fem` (modern Oracle, ADR 0004). Each
// C6 card is single-art — no multi-art CardPrints in this cluster.
// ─────────────────────────────────────────────────────────────────────────────

// ── Sacrifice-land cycle (REUSE C, ADR 0039) ─────────────────────────────────
// "This land enters tapped.\n{T}: Add {X}.\n{T}, Sacrifice this land: Add
// {X}{X}." The fixed-output {T} mana ability and the sacrifice-self {T}+sac
// mana ability (Basal Thrull / Black Lotus shape) share one source. `useStack:
// false` (CR 605.1a). Entering tapped is the self `entersTapped` flag.
...
// ── Storage-land cycle (CAPABILITY H + REUSE I) ──────────────────────────────
// "This land enters tapped.\nYou may choose not to untap this land during your
// untap step.\nAt the beginning of your upkeep, if this land is tapped, put a
// storage counter on it.\n{T}, Remove any number of storage counters from this
// land: Add {X} for each storage counter removed this way."
//
// Three mechanics, all reused: `entersTapped` (self flag); the
// `may-choose-not-to-untap` static ability (CAPABILITY I, FEM C2); and the
// variable counter-removal → variable mana seam (CAPABILITY H,
// `manaChoiceRemovesCounters`, CR 122.6 / 605.1a). The chosen index N removes N
// `storage` counters and adds exactly N mana of the land's colour — no base
// mana (distinct from the Mana Battery's 1 + N). Index 0 removes nothing and
// adds nothing (a legal "any number" choice). The upkeep counter accrues only
// while the land is tapped (the player held it via the optional-untap choice).
...
// ── Rainbow Vale — control-change-on-tap (CAPABILITY B, ADR 0040) ────────────
// "{T}: Add one mana of any color. An opponent gains control of this land at
// the beginning of the next end step." A `useStack:false` tap mana ability
// (CR 605.1a — the mana add is a mana ability) with `manaChoices` for the
// any-colour pick, plus the `armsDelayedTriggerOnTap` rider (ADR 0040) that
// arms the `next-end-step` delayed trigger. The delayed trigger hands the land
// to the SINGLE opponent (2-player; PRD §Out of Scope) via INDEFINITE
// `gainControl` (no condition — Ghazbán Ogre precedent). On the opponent's
// next tap the same rider arms again, handing it back: it ping-pongs.
...
// ── Implements of Sacrifice — sac-self mana (REUSE C, ADR 0039) ──────────────
// "{1}, {T}, Sacrifice this artifact: Add two mana of any one color." The
// any-one-colour choice rides `manaChoices` (Black Lotus shape); the {1} mana
// component rides `cost.mana`; `cost.sacrifice` sacrifices the source.
...
// ── Spirit Shield / Zelyon Sword — tapped-duration buff (REUSE I) ────────────
// "You may choose not to untap this artifact during your untap step.\n{cost},
// {T}: Target creature gets +P/+T for as long as this artifact remains tapped."
// The buff rides `addSourceTappedPTBuff` (Ashnod's Battle Gear shape, CR 611.2),
// held while THIS artifact stays tapped; the optional-untap static ability lets
// the controller hold the buff across turns.
...
// ── Reuse-only sacrifice/tap-effect artifacts ────────────────────────────────

// Aeolipile — "{1}, {T}, Sacrifice this artifact: It deals 2 damage to any
// target." (CR 601.2b "any target"; sac+tap+mana cost; CR 119 damage.)
...
// Balm of Restoration — "{1}, {T}, Sacrifice this artifact: Choose one — • You
// gain 2 life. • Prevent the next 2 damage that would be dealt to any target
// this turn." (CR 119 lifegain; CR 615 prevention.) `ActivatedAbility` has no
// `modes`; the two modes of a single-cost modal ability are expressed as two
// separate activated abilities sharing the same {1},{T},Sacrifice cost — the
// player picks which one to activate (Pyramids precedent, ARN). Each abillity
// pays the full cost (only one resolves), faithful to "choose one".
...
// Conch Horn — "{1}, {T}, Sacrifice this artifact: Draw two cards, then put a
// card from your hand on top of your library." (CR 121.1 draw; CR 700-style
// "then put back"; resolved as draw 2 then a top-of-library choice.)
...
// Draconian Cylix — "{2}, {T}, Discard a card at random: Regenerate target
// creature." (CR 118.3 random-discard cost; CR 701.15 regeneration shield.)
...
// Elven Lyre — "{1}, {T}, Sacrifice this artifact: Target creature gets +2/+2
// until end of turn." (CR 611.2c temporary P/T buff.)
...
// (CR 118.3 random-discard cost; CR 121.1 draw.)
...
// Delif's Cone — "{T}, Sacrifice this artifact: This turn, when target creature
// you control attacks and isn't blocked, you may gain life equal to its power.
// If you do, it assigns no combat damage this turn."
//
// FAITHFUL-TEXT DEFERRAL: the engine has no turn-scoped ARMED "when target
// creature attacks and isn't blocked" rider keyed to a chosen creature
// (resident `ATTACKER_UNBLOCKED` triggers live on permanents/auras, not on a
// fired-then-armed pending effect). The sac+tap cost and target selection are
// expressed; the unblocked-attack payoff is deferred and flagged rather than
// silently dropped. See #573 / PRD #566.
...
// Delif's Cube — "{2}, {T}: This turn, when target creature you control attacks
// and isn't blocked, it assigns no combat damage this turn and you put a cube
// counter on this artifact.\n{2}, Remove a cube counter from this artifact:
// Regenerate target creature."
//
// The cube-counter regeneration ability is fully expressed (CR 122.6
// remove-counter cost + CR 701.15 regeneration). The arm-on-target unblocked-
// attack first ability is a FAITHFUL-TEXT DEFERRAL for the same reason as
// Delif's Cone (no turn-scoped armed attack rider keyed to a chosen creature).
