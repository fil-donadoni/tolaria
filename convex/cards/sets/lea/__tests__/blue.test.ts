...
describe("Control Magic (Aura control-change, CR 613.1b layer 2 + 702.10c)", () => {
    it("on resolve, transfers control of the enchanted creature and sets summoning sickness", () => {
        ...
        // PRD #2064 S4 — an AURA's control change writes no bookkeeping at
        // all. It is generated by a static ability, so `syncLayers2to5` derives
        // it from the board at every read; `baseControllerId` is the CR 108.3
        // controller it collapses back to when the Aura stops applying, and
        // there is no reversal record because there is nothing to reverse.
        expect(bearAfter.controlChanges).toBeUndefined();
        expect(bearAfter.baseControllerId).toBe("p2");
    ...
    it("stacked CMs: removing the MIDDLE entry leaves current controller intact and top pops to owner (CR 108.3)", () => {
        ...
        // PRD #2064 S4 — "removing the middle entry" is no longer an operation:
        // the derivation replays what is STILL on the board from
        // `baseControllerId` in CR 613.7 timestamp order, so a source leaving
        // from anywhere in the order needs no chain patch and the surviving
        // sources' answer is simply recomputed.
        expect(bearMid.controlChanges).toBeUndefined();
        expect(bearMid.baseControllerId).toBe("p1");
...
describe("Steal Artifact (Aura control-change on artifacts, CR 613.1b layer 2)", () => {
    it("on resolve, transfers control of the enchanted artifact (no summoning sickness — artifacts aren't creatures)", () => {
        ...
        // PRD #2064 S4 — derived, not recorded; see the Control Magic case.
        expect(statueAfter.controlChanges).toBeUndefined();
        expect(statueAfter.baseControllerId).toBe("p2");
