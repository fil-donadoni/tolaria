...
export const wheelOfFortune: CardDefinition = {
    ...
    manaCost: { X: 2, R: 1 },
    types: ["Sorcery"],
    // Migrated resolve()→effects[] (ADR 0045, issue #1279): "discards their
    // hand" is now the `discard` Op's bulk whole-hand shape (`cards` omitted)
    // — every card currently in hand, no selection.
    effects: [
        {
            op: "forEach",
            select: { set: "players" },
