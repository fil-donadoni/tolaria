...
const KEYWORD_ABILITIES: MechanicRow[] = [
    ...
    // 702.175 Offspring
    {
        ...
        cr: "702.175",
        status: "planned",
        note: 'CR 702.175a is TWO abilities: "You may pay an additional [cost] as you cast this spell" — word for word Kicker\'s cost half (CR 702.33a), which the shared kickers[] subsystem already expresses — AND "When this permanent enters, if its offspring cost was paid, create a token that\'s a copy of it, except it\'s 1/1", which is the half still to build (issue #2079). Its IDENTITY exists as of ADR 0085 / issue #2078: `keyword: "offspring"` on a kickers[] entry, whose ADDITIONAL_COST_KEYWORDS row (gre/kicker.ts) states countsAsKicked: false — CR 702.33d defines "kicked" over kicker costs alone, so an offspring payment lands in StackItem.unkickedCostPayments and is invisible to wasKicked, the count "kicker" ETB tally, { kickerCount: true }, the kicked target-requirement swap and the spellWasKicked filter, while { additionalCostPaid: "<id>" } still reads it. Until #2079 lands no shipped card may declare the keyword (guard: cards/__tests__/additionalCostKeywords.test.ts).',
    },
    // 702.176 Impending
    {
        id: "impending",
