...
export const EFFECT_OP_REGISTRY: EffectOpRow[] = [
    ...
    },
    {
        ...
        cr: "107.1b",
        binding: "SpellContext.requestNumberChoice",
        note: 'Bare numeric nomination — "choose a number" (issue #1421, gap 6a of tracker #1120). CR 107.1b governs an undefined number: a player instructed to choose one chooses a non-negative integer, and anything needing the value of a number that was never defined reads 0. The chosen value costs nothing and is READ by a later Op through the required `bind` (`manaValueEquals: { ref: "$n" }` — Void\'s "Choose a number. Destroy all artifacts and creatures with mana value equal to that number."). Skins `SpellContext.requestNumberChoice` WITHOUT `payMana`, so unlike its paying sibling it opens no CR 608.2g mana window: there is no mana to make, and `isManaPaymentChoiceWindow` (gre/state.ts) says so explicitly. Shares ONE Pending Choice family with `payVariableMana` (CR 107.3f, issue #1701) — the same `number-pick` kind, the same `numberChoiceRange` authority, the same `submitNumberChoice` mutation, the same client affordance and the same bot candidate generator; the two stay DISTINCT Ops because nominating and nominating-then-paying are different decisions with different bot valuations, which is the coordination both issues recorded. `min` / `max` are optional `EffectValue`s resolved at Op execution time (CR 608.2), so a bound can be read off the board as the ability resolves; a bound resolving to `undefined` is dropped rather than read as 0, because a silent 0 ceiling pins the answer to the floor and looks like the card working. Both omitted is the OPEN-ENDED nomination CR 107.1b describes: `numberChoiceRange` returns an infinite ceiling, the submit validator accepts any integer at or above the floor, the client renders free entry instead of a stepper, and the bot searches a BOUNDED candidate set derived from the board (the highest mana value among battlefield permanents and the chooser\'s hand) — the legal range is infinite, the searched one never is. Deliberately NOT built: a non-numeric nomination vocabulary (a colour, a card type — those are `chooseCategorized` / `nameCard`) and a "number you haven\'t chosen before" style restriction, which no shipped card asks for.',
    },
    {
        op: "if",
        status: "implemented",
