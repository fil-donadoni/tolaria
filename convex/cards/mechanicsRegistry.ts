...
const KEYWORD_ABILITIES: MechanicRow[] = [
    ...
    // 702.35 Madness
    {
        ...
        note: "Cost-system / keyword-cast capability (engine infra, NOT an Effect Script Op): convex/gre/madness.ts (getMadnessCost / markMadnessExiled / windowExpiryTurn / sweepUncastMadness) + CardDefinition.madness (printed madness cost; `Madness {0}` is the empty cost `{}`) + CardInstanceState.madnessExiled. discardToGraveyard (convex/gre/state.ts) redirects a discarded madness card hand→exile (CR 702.35c) and opens a cast-from-exile window via the shared castableFromExileBy permission; getLegalActions (rules.ts) offers the cast at instant speed for the madness cost; castRawManaCost (game.ts) charges the madness cost on the exile cast; sweepUncastMadness (phases.ts cleanup) puts an uncast copy into the graveyard once its window expires (CR 702.35d). CR-simplification (real game-observable deviation, tracked #1198): the 702.35d reflexive trigger is modeled as an impulse cast window (no CR 514.3 priority loop), so (a) a DECLINED card reaches the graveyard at the window's cleanup rather than the instant the trigger resolves — a mid-turn graveyard-count read (Threshold/Delirium) can see one fewer card until then — and (b) a card discarded to hand size at the cleanup step is castable into the following turn (windowExpiryTurn extends it one turn, since cleanup grants no priority) rather than only during the discarding player's end step. Used by Basking Rootwalla, Blazing Rootwalla, Anje's Ravager.",
    },
    // 702.36 Fear
    {
        id: "fear",
...
export const EFFECT_OP_REGISTRY: EffectOpRow[] = [
    ...
    },
    {
        ...
        note: 'Dig to hand (CR 401.4 "look at", issue #984): look at the top `look` cards of a library, put `take` of them (default 1) into hand, and put the rest on the BOTTOM in any order. A thin declarative skin composed of existing primitives (the Stock Up composition generalized), one execution path (ADR 0045): `peekLibraryTop(look)` reveals the top cards; a single suspending `look-distribute` `requestChoice` over exactly those ids (candidateIds = the looked-at top N, projected face-up as `libraryPeek` — never the whole hidden library, the `search-library` over-exposure) drives the unified HAND/BOTTOM pick (count = EXACTLY `keep` to hand); the kept cards move library→hand via `moveCardById`; the remaining looked-at cards are bottomed via `reorderLibraryTop` in the player\'s CHOSEN order, read back via `readOrderedSecond`. SUSPENDS like `choice`/`scryReorder` — the pick is consumed internally, no `bind` read by a later Op. The bottom cards are marked known to the controller via `markKnown` (ADR 0026): the player looked at and PLACED them, so their bottom position is certain and Impulse\'s "in any order" (CR 401.4) is a real choice — the projection exposes them as the contiguous known run from the bottom until a shuffle. A bot/auto path may submit only the hand picks (empty `secondZoneIds`), and then the rest auto-bottom in look order. `player` names whose library; `look` is how many to look at; `take` is how many kept (default 1, clamped to the number looked at). DISTINCT from the exile-and-may-play "impulse draw" (#791): the chosen card enters HAND and the rest go to the bottom — no exile, no play window. Impulse (MMQ) = look 4, take 1.',
    },
    {
        op: "preventDamage",
        status: "implemented",
