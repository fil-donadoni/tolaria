...
export const EFFECT_OP_REGISTRY: EffectOpRow[] = [
    ...
    },
    {
        ...
        note: "Replaces a target land's subtypes for a limited duration (layer 4, CR 305.7, issue #1083). A thin declarative skin over the SpellContext primitive `setSubtypesUntil`, one execution path (ADR 0045). Distinct from `addSubtype` above (which ADDS a subtype INDEFINITELY, keeping the printed ones): this Op REPLACES the target's subtypes outright and reverts at `duration` when one is given; `duration` is OPTIONAL since issue #1746 and an omitted one replaces INDEFINITELY (CR 611.2b — Figure of Destiny's staged respec; Enduring Innocence, issue #2084, uses the indefinite arm with `subtypes: []` for CR 205.1a's correlated-subtype removal) — the \"target land becomes a Swamp / the basic land type of your choice until end of turn\" template (Orcish Farmer / Slimy Kavu precedent `resolve()` closures now composable as a DSL Op; Dream Thrush's \"{T}: Target land becomes the basic land type of your choice until end of turn\" pairs it with `optionChoice`, one mode per basic land type). Was never reserved under its own name in `EFFECT_OP_BACKLOG` — the gap was discovered and closed directly (issue #1083). `subtypes` may be a bare `{ ref }` (issue #3809 — Unnatural Selection's \"becomes that type\"): a chooseCreatureType binding or the reserved `$source.chosenSubtype`; a set made of creature types replaces only creature types (CR 205.1a).",
    },
    {
        op: "animate",
        status: "implemented",
