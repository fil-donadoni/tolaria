...
/** How broad a "you may spend mana as though it were mana of any …" permission
 *  is (CR 609.4b). The two printed breadths differ in exactly one place —
 *  whether COLORLESS is a legal substitution TARGET:
 *
 *    - `"any-type"` — CR 118.14: "mana of any type can be spent" means "as
 *      though it were colorless mana or mana of any color"; CR 106.1b lists six
 *      TYPES (the five colours plus colorless). North Star.
 *    - `"any-color"` — CR 105.1 lists five COLORS, colorless not among them.
 *      Robber of the Rich, Agatha's Soul Cauldron.
 *
 *  Observable consequence: a `{C}` pip (CR 107.4c — "a cost that can be paid
 *  only with one colorless mana") is payable with coloured mana under
 *  `"any-type"` and NOT under `"any-color"`. Snow is outside both — CR 107.4h,
 *  "Snow is neither a color nor a type of mana".
 *
 *  The pairs each breadth authorises are generated by `substitutionsForBreadth`
 *  (`convex/gre/manaColors.ts`); nothing else may enumerate them. */
export type ManaSubstitutionBreadth = "any-color" | "any-type";
...
export interface SpellContext {
    ...
    grantCastFromExile: (
        ...
        opts?: {
            ...
            /** CR 609.4b (issue #2890) — the granting Oracle text also says
             *  "you may spend mana as though it were mana of any color/type to
             *  cast that spell" (Robber of the Rich: `"any-color"`). Scoped to
    ...
    /** CR 611.2c / 613 — creates ONE Continuous Effects Registry entry (ADR
     *  0082, PRD #2064): a continuous effect generated by this resolution, with
     *  its own expiry, that is NOT tied to the resolving object's continued
     *  presence.
     *
     *  The general form the two `grant*Permanent` primitives above are special
     *  cases of, and the only channel that can be source-INDEPENDENT and
     *  condition-GATED at once. Dread Wight's end-of-combat trigger is the
     *  catalogue's conformance case: "each of those creatures doesn't untap
     *  ... for as long as it has a paralyzation counter on it" is one effect
     *  that must outlive Dread Wight AND stop the moment the counter comes off,
     *  which `staticEffects[]` (source-keyed) and `grantStaticAbilityPermanent`
     *  (ungated) could each do only half of.
     *
     *  There is deliberately NO revoke sibling. An entry applies while its
     *  `expiry` says it does and the derivation simply stops producing it
     *  otherwise — revocation is not an operation (PRD #2064).
     *
     *  `id` and `timestamp` are minted by the engine: the id deterministically
     *  (`ce-N`) so a replay reproduces it, and the timestamp from the single
     *  CR 613.7 sequence every layer effect shares. */
    addContinuousEffect: (
     ...
     *  an entry to `state.spellManaSubstitutionGrants`; `getManaSubstitutions`
     *  turns it into `{from,to}` pairs for a payment that names a cast in
     *  progress (never for an activated ability's cost — the Oracle says "that
     *  SPELL's mana cost"), and the cast-payment seam pops one entry when the
     *  substitution was what made the cost payable. Per CR 609.4b the spell's
     *  COST is unchanged; only how it may be paid is. Not idempotent — each
     *  grant buys one spell, so two activations stack. Cleared at CLEANUP
     *  (CR 514.2). */
...
export type EffectOp =
     ...
     *  spell's mana cost") — grant `player` a ONE-SHOT mana-substitution
     *  permission of the given `breadth`. A thin declarative skin over
     *  `SpellContext.grantSpellManaSubstitution`, one execution path (ADR
     ...
     *
     *  Per CR 609.4b this "affects only how the player may pay a cost. It
     *  doesn't change that cost": no cost-modifier, no wire-projection and no
     *  cost-display path is touched. `breadth` picks whether COLORLESS is a
     *  legal substitution target (`ManaSubstitutionBreadth`) — the one place the
     *  two printed wordings differ. Scoped to SPELLS, per the Oracle's "that
     *  spell's mana cost": an activated ability's cost never sees the grant.
     *  Cleared at CLEANUP (CR 514.2). Skipped when the player cannot be
     *  resolved (CR 608.2b).
     *
     *  Distinct from the `mana-substitution` STATIC effect (Sunglasses of Urza),
     *  which is a continuous single `{from,to}` pair scanned off the battlefield
     *  and applies to every cost; this Op is a one-shot, spell-scoped grant. */
    | {
