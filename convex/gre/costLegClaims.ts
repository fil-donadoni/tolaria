...
export const COST_LEG_CLAIMS = {
    ...
    exertThis: {
        ...
        why: "CR 701.43a/b / 602.1a — always payable, so `enumerateAbilityMoves` gates nothing: CR 701.43b lets a permanent be exerted while untapped and lets it be exerted again before its next untap step, which leaves no board state that could refuse the leg. BOTH Bot surfaces pay it through the one `payExertActivationCost` authority (`gre/exert.ts`), so the Bot prices the missed untap instead of getting the ability for free: an `activate-ability` move here, and — since issue #3359 — a MANA ability's tap plan in the search's one `applyTapPlanInSearch` (`searchTapPlan.ts`, issue #4444), which resolves the exerting option per `manaChoiceIndex` through `manaTapExertsSource` exactly as `tapSourceIntoPayment` does. The mana ability is the shape the only shipped carrier (Arena of Glory) uses.",
        autoPayable: false,
    },
    returnThisToHand: {
        paidBy: {
