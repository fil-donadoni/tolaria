/**
 * Blade-scenario suite — the shared runner behind the shard spec files
 * (issue #1427, PRD #1423; sharded by issue #4482).
 *
 * NOT part of `bun run test:app`. Run the suite with:
 ...
 *                                   stretch check plus a promotion hint, never
 *                                   as a red `must` check (issue #1517).
 *
 * The tier is selected by the `BLADE_TIER` env var (default `must`) so both
 * modes share one runner and one config — see `vitest.blade.config.ts`.
 *
 * Each `blade.shard-N.spec.ts` calls `registerBladeShard(N)`: shard N runs the
 * entries `bladeShardOf` assigns it (`../shard.ts`), so the tier spreads over
 * the config's workers instead of running serially in one file. Shard 0 also
 * carries the registry-integrity and determinism suites — they are cheap and
 * need to run exactly once.
 */
...
const tierScenarios = bladeScenariosForTier(TIER);

/** Register shard `shard` (0-based) of the blade suite in the calling spec file. */
export function registerBladeShard(shard: number): void {
    const scenarios = bladeShardSlice(tierScenarios, shard);
            ...
            it("a beyond-budget entry classifies its cause with a budget it actually exceeds", () => {
                ...
                for (const s of BLADE_SCENARIOS) {
                    ...
                    if (s.beyondBudget.passesAt) {
                        ...
                        // passing budget to record — the type says so, no measurement
                        // could. `horizon` (ADR 0102, issue #3138): when the payoff
                        // sits behind a LOOP, ~40 plies of identical activations
                        // separate the position from lethal and what clears it is the
                        // CR 732 shortcut Move (PRD #2687), not more iterations.
                        // `branching` and `hidden-information` name a genuine compute
                        // shortfall that more search eventually clears, so they must
                        // carry the budget that clears it.
                        expect(
                            ["valuation", "horizon"],
                            `${s.label}: omitting passesAt is only valid for cause "valuation" or "horizon"`
                        ).toContain(s.beyondBudget.cause);
                        // ...and `horizon` still owes a NUMBER (issue #3138 review).
                        // "No budget clears this" is a claim about a search that was
                        // run; `valuation` is the one cause whose type doc discharges
                        // that claim a priori. Everything else must name the highest
                        // budget it actually swept, or an entry written with no sweep
                        // at all is green — `note` is the field this spec deliberately
                        // refuses to check, so it cannot be the evidence.
                        if (s.beyondBudget.cause !== "valuation") {
                            expect(
                                s.beyondBudget.sweptTo?.iterations,
                                `${s.label}: omitting passesAt on cause "${s.beyondBudget.cause}" requires sweptTo — the highest budget swept, which must exceed the declared budget`
                            ).toBeGreaterThan(s.budget.iterations);
                        }
                    }
        ...
        describe("blade suite — determinism (acceptance criterion, #1427)", () => {
            ...
            it.each(PROBE_LABELS)(
                ...
                (label) => {
                    ...
                    expect(
                        probe,
                        `registry entry "${label}" not found`
                    ).toBeDefined();
    ...
    describe(`blade suite — ${TIER} tier${VARIANT_SUFFIX} — shard ${shard + 1}/${BLADE_SHARDS}`, () => {
        if (scenarios.length === 0) {
            it(`has no ${TIER} scenarios in this shard`, () => {
            ...
            it(scenario.label, () => {
                ...
                if (TIER === "stretch") {
                    ...
                    const detail = result.ok
                        ? result.seeds
                              .map(
                                  (s) => `seed ${s.seed}: ${s.moveDescription}`
                              )
                              .join("; ")
