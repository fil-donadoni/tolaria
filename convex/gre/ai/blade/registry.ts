...
export const BLADE_SCENARIOS: BladeScenario[] = [
    ...
    },
    {
        ...
        beyondBudget: {
            cause: "horizon",
            note: "The payoff sits behind a LOOP, not behind a mis-valuation (ADR 0102, issue #3138): Splinter Twin grants the host '{T}: create a token that's a copy of this creature, except it has haste', and the copied Deceiver Exarch's own ETB trigger untaps the original, so activate → trigger → resolve → activate repeats for ~40 plies of identical moves before the evaluation sees lethal. A 1-ply-greedy rollout never walks that, and no budget a browser Worker gets buys it: measured post-removal on these five seeds at 400 / 1200 / 4000 / 12000 iterations, this entry is chosen 0/5 at every budget (the sibling activation entry's 2/5 at 4000 is rollout noise — it is back to 0/5 at 12000), which is why `passesAt` is omitted. What collapses the loop to ONE Move whose payoff the evaluation sees a single ply later is the CR 732 shortcut primitive (PRD #2687); the per-card boost layer that used to sit here was deleted by issue #3138 because, being a function of the state, it scored identically before and after the activation and saturated the material signal, so more search converged AWAY from the combo.",
        },
