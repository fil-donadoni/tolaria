...
import { stackRebuildMismatch, type SeatOf } from "./stackFingerprint";
import type { VerdictCandidate } from "./types";
import type { StackBlocker } from "../../scenarioBuilder";
 ...
 *
 * A retired member's NAME is never reused: a historical sweep table counting
 * it would then mean two different things. `stack-not-journalled` (issue
 * #3480) is gone with the journal, and its two successors below are new names
 * (issue #3514).
 *
 *  - `stack-mid-resolution` — an object on the stack is PARTWAY THROUGH
 *    resolving (`resolutionStep` / `collectedChoices`). The spec declares an
 *    object announced, never a half-resolved one, and the collected answers
 *    are not something a rebuild can hold.
 ...
 *    refusal names the item and the FIELD that blocked it.
 ...
 *    not rebuild it.
 *  - `stack-rebuild-mismatch` — every object was named, and the rebuilt stack
 *    is still not the one in play (`stackFingerprint.ts`: seat, name, kind,
 *    targets and `x`).
...
export type LoweredDecision = {
    /** The position, lowered from the board the search ran on — the LIVE
     *  board, stack included (`spec.stack`, ADR 0127). */
    spec: ScenarioSpec;
...
/** `opp Impulse spell: triggerEvent` — how a refusal names what blocked it. */
export function formatStackBlocker(blocker: StackBlocker): string {
    return `${blocker.item}: ${blocker.field}`;
}
...
export function lowerDecision(
    ...
    const liveSeatOf: SeatOf = (playerId) =>
        playerId === botId
            ? "me"
    ...
    // CR 608.3 — an object PARTWAY through resolving. The declared stack holds
    // announced objects, never a half-resolved one, and `collectedChoices` are
    // answers already given inside this resolution. Its own kind, checked
    // before the lowering runs: it is structural, where `stack-not-lowerable`
    // is a vocabulary gap a later slice can close, and counting the two
    ...
    if (midResolution.length > 0) {
        return {
            ...
            dropped: [],
            error: `${midResolution.length} object(s) on the stack are partway through resolving (CR 608.3) — a declared stack holds announced objects, never a half-resolved one`,
        };
    }
    ...
    // CR 405.1 (issue #3514, ADR 0127) — the stack is lowered as DECLARED
    // objects, and `specFromState` withholds the whole of it when any one
    // object carries a fact the spec cannot name. Rebuilding anyway would be
    // the same board with the spell simply gone, which can leave the candidate
    // list IDENTICAL (pass, and whatever the Bot could do anyway) while the
    // position is materially different — the Bolt that was about to kill a
    ...
    if (blockers.length > 0) {
        return {
            ok: false,
            kind: "stack-not-lowerable",
            dropped,
            error: `the stack could not be declared in the scenario — ${blockers
                .map(formatStackBlocker)
                .join("; ")}`,
        };
    }
    ...
    // board differs. The same argument the stack site above makes, applied
    ...
    // sentinel, which is registered by id and not by name. Placing it under
    // the sentinel's display name used to hand the builder a spec it threw on
    // (`rebuild-threw`, "Card not found by name: Face-down creature"), and
    // leaving it out would judge a board missing a card. Titled
    // `lowering-threw` because that is where the cause is: the lowering.
    const unnamed = dropped
        .filter((note) => note.startsWith(HIDDEN_IDENTITY_DROPPED_PREFIX))
        .map((note) =>
            note
                .slice(HIDDEN_IDENTITY_DROPPED_PREFIX.length)
                .split(" — ")[0]
                .trim()
        );
    if (unnamed.length > 0) {
    ...
    // Did the rebuild reproduce the STACK? `different-decision` below compares
    // candidate LISTS, and a candidate list is exactly what a wrong stack can
    // leave unchanged: a Bolt pointed at the undamaged of two same-named Bears
    // adds no move and removes none. The comparison reads what the lowering
    // did NOT write — the observable facts of each target (`stackFingerprint.ts`)
    // — so it cannot pass by construction. It fails CLOSED, like every other
    // check here, and runs on every decision: an empty stack against an empty
    // rebuild agrees trivially, and a stack the rebuild invented does not.
    const rebuiltSeatOf: SeatOf = (playerId) =>
        playerId === seatId
            ? QUIZ_SEAT
            : rebuilt.players.some((p) => p.id === playerId)
              ? "opp"
              : null;
    const stackDiffers = stackRebuildMismatch(
        position,
        liveSeatOf,
        rebuilt,
        rebuiltSeatOf
    );
    if (stackDiffers !== null) {
        return {
            ok: false,
            kind: "stack-rebuild-mismatch",
            dropped,
            error: `the declared stack rebuilt a different stack: ${stackDiffers}`,
        };
    }
