...
/** Whether `move` is the BOT's own sacrifice of a standing permanent to make a
 *  target player discard and nothing else (`abilityIsDiscardExchange`), in ANY
 *  window (issue #4276).
 *
 *  The sibling of `isRemovalExchangeSacrifice`, on the same terms: the
 *  variants (a victim × target grid) outnumber `pass` in the rollout's random
 *  draw and open as tree children at the node after the cast, dragging the
 *  cast edge under `pass`'s — two creatures whose only ability is a
 *  sacrifice-for-discard outlet were `never-chosen` while the same bodies
 *  without it were cast. The bot's own moves only and never at the root, so
 *  a discard the bot could take NOW stays a scored option and the opponent's
 *  stays in the tree.
 *
 *  Per-card-agnostic, never a card name (ADR 0102). */
export function isDiscardExchangeSacrifice(
    state: GameState,
    pid: string,
    botId: string,
    move: Move
    ...
    return isSacrificeConversionWhere(
        ...
        pid,
        move,
        (ability) => ability.useStack && abilityIsDiscardExchange(ability)
    );
}
...
/** Whether `move` is the BOT's own sacrifice of a standing permanent to draw
 *  cards and nothing else (`abilityIsDrawExchange`), in ANY window (issue
 *  #4279).
 *
 *  The sibling of `isDrainExchangeSacrifice`, on the same terms: the variants
 *  (a victim × target grid) outnumber `pass` in the rollout's random draw and
 *  open as tree children at the node after the cast, dragging the cast edge
 *  under `pass`'s. The bot's own moves only and never at the root, so a
 *  sacrifice-draw the bot could take NOW stays a scored option and the
 *  opponent's stays in the tree.
 *
 *  Per-card-agnostic, never a card name (ADR 0102). */
export function isDrawExchangeSacrifice(
    state: GameState,
    pid: string,
    botId: string,
    move: Move
    ...
    return isSacrificeConversionWhere(
        ...
        pid,
        move,
        (ability) => ability.useStack && abilityIsDrawExchange(ability)
    );
}
                ...
                (k) =>
                    ...
                    !(
                        ...
                        (isSourceConfinedSacrificeConversion(
                            ...
                            isDiscardExchangeSacrifice(
                                world,
                                pid,
                                botId,
                                k.move
                            ) ||
                            isDrainExchangeSacrifice(
                                world,
                                pid,
                                botId,
                                k.move
