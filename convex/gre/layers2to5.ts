...
function layers2to5EffectsFor(
    ...
    const pushSourceEffects = (
        ...
        // timestamp of the object the ability is on. An UNSTAMPED source has
        // not begun applying and is skipped rather than derived at 0: an effect
        // with no timestamp has no position in the layer at all.
    ...
    // (Aladdin, Old Man of the Sea, Ray of Command) and by an aura whose
    // `control-change` static ability the board walk above already covers. Only
    // the ledger rows are read here; a row keyed to a live source that declares
    // a `control-change` static effect is that source's DERIVED entry and is
    // skipped, or the change would apply twice.
    for (const change of instance.controlChanges ?? []) {
        if (sourceDeclaresControlChange(state, change.auraId)) continue;
...
/** Whether `sourceId` names a live battlefield permanent whose card declares a
 *  `control-change` static ability — i.e. whether the board walk already
 *  derives this control change, making the instance's ledger row a duplicate.
 *
 *  A ledger row is written for BOTH arms today (`applyControlChange` is the one
 *  primitive, and `applyAuraControlChange` calls it), so the two channels would
 *  otherwise both produce an entry for a Control Magic. The aura's is the one
 *  that must survive: it is re-derived per read, so it ends the moment the aura
 *  leaves or its `applies` stops holding, which the ledger row cannot see. */
function sourceDeclaresControlChange(
    state: LayerStateView,
    sourceId: string
): boolean {
    for (const player of state.players) {
        for (const card of player.battlefield) {
            ...
            return sourceStaticEffects(view).some(
                (e) => e.kind === "control-change"
            );
        }
    }
    return false;
