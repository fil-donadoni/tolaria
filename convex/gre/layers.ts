...
export function emblemAsStaticSource(emblem: EmblemInstance): PermanentView {
    return {
        ...
        // CR 613.7a/d — the emblem's own minted layer timestamp, so its
        // continuous effects order against the board's like any other source's
        // (PRD #2064 S6b-part-2 replaced the creation-order floor). An emblem
        // with none is a hand-built fixture or a state persisted before that
        // slice — `migrateLegacyLayer2to5Ledgers` stamps the latter on load —
        // and derives at 0, "earliest in the layer", rather than being skipped:
        // layer 6 skips an unstamped SOURCE, and an emblem that vanished from
        // the walk would ship every emblem-granted keyword inert.
        staticSeq: emblem.staticSeq ?? 0,
    } as PermanentView;
}
...
function layer7EffectsFor(
    ...
    // a counter's CR 613.7c timestamp is not stored (one entry per TYPE, and
    // every counter of a kind shares a stamp, so they cannot interleave), and a
    // "for as long as the source remains tapped" buff is keyed to the source's
    // tapped state rather than to a moment. Both are sublayer 7c, which SUMS
    // (CR 613.4c), so their relative order carries no information at all — the
    // ordinal below is a stable identity, not a position, which is why PRD #2064
    // S6b-part-2 could delete the floor it used to start from without changing
    // any answer. A 7a/7b provenance could not be handled this way, and none is
    // derived here.
    let ordinal = 0;
    ...
    //    bonus twice.
    // 2. ORDER. Since PRD #2064 S6b-part-2 a derived SOURCE entry carries the
    //    source's own minted stamp (CR 613.7a), so it orders against a stored
    //    entry by when each began applying rather than always losing to it.
    //    That is invisible in the summing sublayer 7c but decides the winner in
    //    the last-wins sublayers 7a and 7b.
