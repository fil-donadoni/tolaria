...
**This is now ENFORCED, not advised — `.claude/hooks/spawn-guard.sh` denies an `Agent` spawn that omits `model` (except `fork`, which cannot honour it).** The prose rule above sat in this file for months and did not hold: `bun run loop:scorecard` measured **243 of 1,979 spawns (12%) over 30 days** running at an inherited tier. A rule that CAN be enforced mechanically belongs in a hook, not in a paragraph.

**The same hook requires a ROLE-PREFIXED `description`** — `implement` / `review` / `fixup` / `investigate` / `research` / `verify` / `migrate` / `audit`, then the rest of the sentence as you like (`"investigate where pendingChoices is projected"`). It is what attributes a spawn's tokens to a role in the scorecard; without it, **55% of agent tokens** were unattributable, which is not an analysis gap to fix downstream — the information was never recorded. The role deliberately lives in the description rather than in dedicated agent DEFINITIONS: a definition costs resident prompt tokens on every request (the weight issue #2189 removed) to express per-spawn metadata, not a capability.
