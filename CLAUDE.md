...
`bun scripts/pr-merge.ts <PR#>` — never a second `land` on the OPEN PR, which
re-pays the whole gate — **then re-run `land` once the PR reads MERGED** and it
runs the post-merge housekeeping alone (no rebase, no lane gate, no merge;
issue #4159). `pr-merge` merges and nothing else, so skipping that second call
loses the scenario seed, `gaps:sync` and the fast-forward in silence — the
issue #3253 failure, verbatim. Per-command hatch:
`TOLARIA_ALLOW_MANUAL_MERGE=1`. A `skin`-lane PR owes
