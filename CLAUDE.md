...
7. **Preset scenario** — For any new card or gameplay feature, add a scenario so the user can load it one-click from the Debug panel and exercise the feature end-to-end (ADR 0044, issue #770: scenarios are `debugScenarios` DB rows, not a code literal). Fastest path: open the Debug panel → Scenarios → "Save scenario" and insert a spec (label + JSON matching `debugSetupScenario`'s args minus `gameId`) via `saveDebugScenario` (`convex/debugScenarios.ts`) — a runtime DB insert, not a tracked code edit, so it never touches a shared file. Choose cards/zones/phase/`landCount` that hit the golden path (and ideally a key edge case). Skip only for pure refactors with no user-visible behavior change.
