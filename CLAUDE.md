...
7. **Preset scenario** — For any new card or gameplay feature, add a scenario so the user can load it one-click from the Debug panel and exercise the feature end-to-end (ADR 0044, issue #770: scenarios are `debugScenarios` DB rows, not a code literal). There are **two authoring paths — pick by whether you have a live browser**:
    - **Automated / headless work (agents, `/process-gh-issues`) — MANDATORY code path.** A subagent has no browser and cannot open the Debug panel, so it authors the scenario as a **code entry**: append one `{ label, spec }` object (spec = `debugSetupScenario`'s args minus `gameId`) to the `NEW_MECHANIC_SCENARIOS` array in `convex/debugScenarios.ts`. This is an **append-only registration point** (PR-tracked, merge-train absorbs the trivial append-conflict) that the `seedNewMechanicScenarios` internalMutation seeds into `debugScenarios` rows on deploy. This is the ONLY scenario path an agent may use — the runtime DB insert below is unreachable headless, so "skip the scenario" is never the fallback.
